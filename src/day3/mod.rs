...
fn parse_claim(raw_claim: String) -> Claim {
    ...
    let mut xy: Vec<&str> = parts.next().unwrap().trim_matches(':').split(',').collect();
    let x :i32 = xy[0].parse().unwrap();
    let y: i32 =  xy[1].parse().unwrap();
    let mut dims: Vec<&str> = parts.next().unwrap().split('x').collect();
    let width: i32 = dims[0].parse().unwrap();
    let height: i32 = dims[1].parse().unwrap();
    let lrx = x+width-1;
    let lry = y+height-1;

    return Claim { x, y, width,height, lrx, lry};
}

fn count_overlaps(filename: String, dims :i32) -> i32 {
    let claims : Vec<Claim> = utility::load_strings(filename).iter().map( | raw| parse_claim(raw.to_string())).collect();
    let size = dims*dims;

    let mut field = vec![0; size as usize];

    claims.iter().for_each(|claim| {add_claim(claim, &mut field, dims)});
...
mod tests {
    ...
    fn should_parse_claim_string() {
        ...
        let mut expected = Claim { x: 1, y: 3, width:4,height:4, lrx:4, lry:6};
    ...
    fn should_increment_by_claim() {
        let mut sample = Claim { x: 1, y: 1, width:1,height:1,  lrx:0, lry:0};
    ...
    fn should_solve_sample() {
        assert_eq!(count_overlaps("./src/day3/test_claims.txt".to_string(), 8), 4);
    }

        #[test]
    fn should_solve_clean_sample() {
        assert_eq!(find_no_overlaps("./src/day3/test_claims.txt".to_string()), "3");
    }
